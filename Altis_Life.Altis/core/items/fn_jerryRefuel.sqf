...
if(isNull _vehicle) exitWith {hint "Du musst dein Fahrzeug anschauen um zu Tanken!"};
if(!(_vehicle isKindOF "LandVehicle") && !(_vehicle isKindOf "Air") && !(_vehicle isKindOf "Ship")) exitWith {};
if(player distance _vehicle > 7.5) exitWith {hint "Dein Fahrzeug ist zu weit weg!"};

if(!([false,"fuelF",1] call life_fnc_handleInv)) exitWith {};
life_action_inUse = true;
_displayName = getText(configFile >> "CfgVehicles" >> (typeOf _vehicle) >> "displayName");

_upp = format["Tanken %1",_displayName];
...
titleText[format["Du hast %1 getankt",_displayName],"PLAIN"];
