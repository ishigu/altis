...
if(_target getVariable ["Reviving",ObjNull] == player) exitWith {hint "Diese Person wird bereits wiederbelebt";};
if(player distance _target > 5) exitWith {}; //Not close enough.

//Fetch their name so we can shout it.
_targetName = _target getVariable["name","Unknown"];
_title = format["Belebe %1 wieder",_targetName];
...
if(_target getVariable["Revive",FALSE]) exitWith {hint "Entweder ist diese Person respawned oder wurde bereits wiederbelebt."};
if((player getVariable["restrained",false])) exitWith {life_action_inUse = false;};
if(!isNil "_badDistance") exitWith {titleText["Du bist zu weit weg von der Person.","PLAIN"]; life_action_inUse = false;};
if(life_interrupted) exitWith {life_interrupted = false; titleText["Akton abgebrochen","PLAIN"]; life_action_inUse = false;};
