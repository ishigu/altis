...
if(life_cash < life_ticket_val) exitWith
{
	if(life_atmcash < life_ticket_val) exitWith 
	{
		...
		[[1,format["%1 hat zu  wenig Geld um das Ticket zu bezahhlen.",name player]],"life_fnc_broadcast",life_ticket_cop,false] spawn life_fnc_MP;
		closeDialog 0;
	};
	...
	[[0,format["%1 hat das Ticket in Hoehe von $%2 bezahlt",name player,[life_ticket_val] call life_fnc_numberText]],"life_fnc_broadcast",west,false] spawn life_fnc_MP;
	closeDialog 0;
};
