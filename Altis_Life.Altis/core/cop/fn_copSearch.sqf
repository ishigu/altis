...
if(count _invs > 0) then
{
	...
	[[0,format["%1 hatte illegale Ware im Wert von $%2 bei sich.",name _civ,[_illegal] call life_fnc_numberText]],"life_fnc_broadcast",west,false] spawn life_fnc_MP;
}
	else
{
	_inv = "Keine illegale Ware";
};
if(!alive _civ || player distance _civ > 5) exitWith {hint format["Konnte %1 nicht durchsuchen", name _civ]};
//hint format["%1",_this];
...
if(_robber) then
{
	[[0,format["%1 wurde als Bankraeuber ausgemacht!",name _civ]],"life_fnc_broadcast",true,false] spawn life_fnc_MP;
};