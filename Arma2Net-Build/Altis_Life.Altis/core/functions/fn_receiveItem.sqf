...
if(_diff != (parseNumber _val)) then
{
	...
	{
		hint format["%1 hat dir %2 aber du kannst nur %3 tragen, also hast du %4 zurueckgegeben.",_from getVariable["realname",name _from],_val,_diff,((parseNumber _val) - _diff)];
	...
	else
{
	if(([true,_item,(parseNumber _val)] call life_fnc_handleInv)) then
	{
		...
		hint format["%1 hat dir %2 %3 gegeben",_from getVariable["realname",name _from],_val,_type];
	}
		else
	{
		[[_from,_item,_val,_unit,false],"life_fnc_giveDiff",_from,false] spawn life_fnc_MP;
