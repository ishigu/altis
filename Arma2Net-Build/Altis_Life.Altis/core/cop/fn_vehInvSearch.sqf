...
if(count _vehicleInfo == 0) exitWith {hint "Das Fahrzeug ist leer"};
...
if(_value > 0) then
{
	[[0,format["Ein Fahrzeug wurde durchsucht, welches illegale Ware in Hoehe von $%1 geladen hatte.",[_value] call life_fnc_numberText]],"life_fnc_broadcast",true,false] spawn life_fnc_MP;
	life_atmcash = life_atmcash + _value;
	_vehicle setVariable["Trunk",[],true];
}
	else
{
	hint "Keine illegale Ware gefunden.";
};