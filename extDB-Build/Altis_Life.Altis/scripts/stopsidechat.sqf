 ...
 * stopsidechat.sqf
 *
 * Author: someone from the DayZ community
 */
[] spawn {
			...
			if (ctrlText ((findDisplay 63) displayCtrl 101) == "Side Channel") then 
			{
				...
				if (disconnect_me == 0) then {
					if (isNil "warn_one") then {
						warn_one = true;
						systemChat ("Im Side-Chat wird nicht gesprochen. Das ist die erste Warnung!");
						[] spawn DS_slap_them;
				...
				if (disconnect_me == 1) then {
					if (isNil "warn_two") then {
						warn_two = true;
						systemChat ("Im Side-Chat wird nicht gesprochen. Das ist die zweite Warnung!");
						[] spawn DS_slap_them;
						//["beat04"] spawn DS_really_loud_sounds;
						["IM SIDE-CHAT WIRD NICHT GESPROCHEN"] spawn DS_double_cut;
					};
				...
				if (disconnect_me == 2) then {
					if (isNil "warn_three") then {
						warn_three = true;
						systemChat ("Im Side-Chat wird nicht gesprochen. Das ist die LETZTE Warnung!");
						systemChat ("Noch ein Mal und du wirst eingefroren und danach gekickt!");
						[] spawn DS_slap_them;
						//["beat04"] spawn DS_really_loud_sounds;
						["IM SIDE-CHAT WIRD NICHT GESPROCHEN"] spawn DS_double_cut;
					};
				...
				if (disconnect_me >= 3) then {
					if (isNil "warn_last") then {
						...
						systemChat ("Du wurdest gewarnt. Denk nochmal eine Minute darüber nach, ob es dir das wert war.");
						sleep 60;
