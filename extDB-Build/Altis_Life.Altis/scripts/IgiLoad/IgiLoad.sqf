...
if (isnil "IL_Variables") then
{
	...
	IL_Num_Slots_OFFROAD = -0;
	IL_Num_Slots_VAN = -0;
	IL_Num_Slots_MOHAWK = -0;
	IL_num_Slots_CHINOOK = -0;//-0;
	IL_Num_Slots_KAMAZ = -0;
	IL_Num_Slots_TEMPEST = -0;
	IL_Num_Slots_HEMTT = -10;
	IL_Num_Slots_MH9 = -0;
	IL_Num_Slots_C130J = -0;
	IL_Num_Slots_C17 = -0;
	...
	IL_Supported_Vehicles_HEMTT = ["B_Truck_01_mover_F"];
	...
	IL_Supported_Vehicles_All = IL_Supported_Vehicles_HEMTT;
	...
	IL_Supported_KAMAZ = ["O_Truck_02_transport_F", "O_Truck_02_covered_F"];
	IL_Supported_SUV = ["C_SUV_01_F"];
	IL_Supported_HEMTT = ["B_Truck_01_transport_F", "B_Truck_01_transport_F"];
...
if !((_obj_main_type in (IL_Supported_Vehicles_All)) || (_obj_main == Player)) exitwith
{
	if (IL_DevMod) then
	{
		Player globalChat Format["Fahrzeug: ""%1"" ist nicht möglich.", _obj_main];
	};
};
		...
		{
			{
				...
				else
				{
					...
					if ((_box_num > _slot_num) && !_done) then
					{
						...
						{
							[_v, Format ["""%1"" wurde aufgeladen. Freie Plätze: ""%3"".", getText(configFile >> "cfgVehicles" >> typeOf _x >> "displayName"), getText(configFile >> "cfgVehicles" >> typeOf _v >> "displayName"), abs(_slot_num - _counter)]] call IL_Vehicle_Chat;
						}
						else
						{
							[_v, Format ["""%1"" wurde aufgeladen. keine Plätze mehr frei.", getText(configFile >> "cfgVehicles" >> typeOf _x >> "displayName"), getText(configFile >> "cfgVehicles" >> typeOf _v >> "displayName")]] call IL_Vehicle_Chat;
						};
					}
					else
					{
		...
		{
			{
				...
				if (_x getVariable "doors" == _doors) then
				{
					...
					{
						[_v, Format ["Abladung des ""%1"" started", getText(configFile >> "cfgVehicles" >> typeOf _x >> "displayName"), getText(configFile >> "cfgVehicles" >> typeOf _v >> "displayName")]] call IL_Vehicle_Chat;
						_done = true;
						_skip = false;
						_zload = (_v getVariable "zload") + (_x getVariable "zload_cargo");
						_cargo_offset = (_v getVariable "cargo_offset") + (_x getVariable "cargo_offset");
						...
						[_v, Format ["""%1"" wurde abgeladen. Freie Plätze: ""%3"".", getText(configFile >> "cfgVehicles" >> typeOf _x >> "displayName"), getText(configFile >> "cfgVehicles" >> typeOf _v >> "displayName"), _free_slots]] call IL_Vehicle_Chat;
						sleep 1;
					};
				};
				if (_done) exitWith {};
...
if (_obj_main_type in IL_Supported_Vehicles_HEMTT) then
{
	...
	_obj_main addAction [
	"<img image='scripts\IgiLoad\images\load.paa' /><t color=""#007f0e"">  Fahrzeug aufladen</t>",
	{
	...
	_obj_main addAction [
	"<img image='scripts\IgiLoad\images\unload.paa' /><t color=""#ff0000"">  Fahrzeug abladen</t>",
	{
	...
	_obj_main addAction [
	"<t color=""#ffffff"">Beladung von außen:</t> <t color=""#2C6700"">AN</t>",
	{
	...
	_obj_main addAction [
	"<t color=""#ffffff"">Beladung von außen:</t> <t color=""#0000ff"">AUS</t>",
	{
...
{
	Player globalChat Format["Fahrzeug: ""%1"" ist nicht ladbar.", _obj_main_type];
};
