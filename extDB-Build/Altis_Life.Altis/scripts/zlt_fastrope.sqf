...
#define STR_TOSS_ROPES "Seile werfen"
#define STR_FAST_ROPE "Abseilen"
#define STR_CUT_ROPES "Seile trennen"
...
zlt_rope_helis = ["O_Heli_Light_02_unarmed_F","O_Heli_Light_02_F","B_Heli_Transport_01_F","B_Heli_Transport_01_camo_F","O_Heli_Attack_02_F","O_Heli_Attack_02_black_F","I_Heli_Transport_02_F","B_Heli_Light_01_F","I_Heli_light_03_F","I_Heli_light_03_unarmed_F"];
zlt_rope_helidata = 
[
	[
...
zlt_fnc_ropes_cond = {
	_veh = vehicle player;
	_flag = (_veh != player) and (player == driver vehicle player) and {(not zlt_mutexAction)} and !(player getVariable "restrained") and {count (_veh getvariable ["zlt_ropes", []]) == 0} and { (typeof _veh) in zlt_rope_helis } and {alive player and alive _veh and (abs (speed _veh) < MAX_SPEED_ROPES_AVAIL ) };
	_flag;
...
player addAction["<t color='#00ff00'>"+STR_FAST_ROPE+"</t>", zlt_fnc_fastrope, [], 15, false, false, '','not zlt_mutexAction and count ((vehicle player) getvariable ["zlt_ropes", []]) != 0 and player != driver vehicle player && !(player getVariable "restrained")'];
