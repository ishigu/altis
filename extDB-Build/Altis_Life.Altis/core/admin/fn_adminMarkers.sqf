#include <macro.h>
/*
	File: fn_adminMarkers.sqf
	Sourced from Lystics Player Markers Loop
*/
if(__GETC__(life_adminlevel) < 1) exitWith {closeDialog 0; hint localize "STR_ANOTF_ErrorLevel";};
life_markers = !life_markers;
if(life_markers) then {
	PlayerMarkers = [];
	VehicleMarkers = [];
	FinishedLoop = false;
	hint localize "STR_ANOTF_MEnabled";
	while{life_markers} do {
		{
			if !(_x in allUnits) then {
				deleteMarkerLocal str _x;
			};
		} forEach PlayerMarkers;
		_vehCount = 0;
		{
			if !(_x in vehicles) then {
			deleteMarkerLocal str _vehCount;
			};
			_vehCount = _vehCount +1;
		} forEach VehicleMarkers;
		PlayerMarkers = [];
		{
			if(alive _x && isplayer _x) then {
				deleteMarkerLocal str _x;
				_pSee = createMarkerLocal [str _x,getPos _x];
				_pSee setMarkerTypeLocal "mil_triangle";
				_pSee setMarkerPosLocal getPos _x;
				_pSee setMarkerSizeLocal [1,1];
				_pSee setMarkerTextLocal format['%1',_x getVariable["realname",name _x]];
				_pSee setMarkerColorLocal ("ColorRed");
				PlayerMarkers = PlayerMarkers + [_x];
			};
		} forEach allUnits;
		VehicleMarkers = [];
		_vehCount = 0;
		{
		_class = getText(configFile >> "CfgVehicles" >> (typeOf _x) >> "vehicleClass");
			if(alive _x && (_class in ["Car","Air","Ship","Armored","Submarine"]) ) then {
				deleteMarkerLocal str _vehCount;
				_pSee = createMarkerLocal [str _vehCount,getPos _x];
				_pSee setMarkerTypeLocal "mil_triangle";
				_pSee setMarkerPosLocal getPos _x;
				_pSee setMarkerSizeLocal [1,1];
				_pSee setMarkerTextLocal format['%1',[typeOf _x] call life_fnc_fetchVehInfo select 3];
				_pSee setMarkerColorLocal ("ColorOrange");
				VehicleMarkers = VehicleMarkers + [_x];
				_vehCount = _vehCount +1;
			};
		} forEach vehicles;
	sleep 0.2;
};
FinishedLoop = true;
} else {
	if(isNil "FinishedLoop") exitWith {};
	hint localize "STR_ANOTF_MDisabled";
	waitUntil{FinishedLoop};
	{
		deleteMarkerLocal str _x;
	} forEach PlayerMarkers;
	_vehCount = 0;
	{
		deleteMarkerLocal str _vehCount;
		_vehCount = _vehCount +1;
	} forEach VehicleMarkers;
};