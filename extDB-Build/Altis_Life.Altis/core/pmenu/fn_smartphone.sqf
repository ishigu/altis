...
*/
private["_display","_units","_type","_data","_rowData","_msg","_tar"];
...
_cMessageHeader ctrlSetText format["Von/An:              Nachricht:"];
...
switch(_type) do
{
	...
	case 1:
	{
		...
		_rowData = [_data select 0, _data select 1, _data select 2, _data select 3, _data select 4];
		_tar = format["[E] %1",_data select 3];
		if ((getPlayerUID player) == (_data select 0)) then { _tar = format["[G] %1",_data select 4]; }; // [E] -> Empfangen, [G] -> Gesendet
		_cMessageList lnbAddRow[_tar,format["%1 ...",_msg]];
