...
if(visibleMap) then {
	{
		if(_x call life_fnc_isADAC && (!(_x getVariable ["hideNametag",false])) && (!(_x getVariable ["blindfolded",false]))) then 
		{
		_adacs set[count _adacs,_x];
		};
	} foreach playableUnits; //Fetch list of ADACs
